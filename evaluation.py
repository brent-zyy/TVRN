...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    max_n_word = 0
    for i, (images, captions, lengths, Iou, depend, ids, tokens) in enumerate(data_loader):
    ...
    for i, (images, captions, lengths, Iou, depend, ids, tokens) in enumerate(data_loader):
        ...
        for j, nid in enumerate(ids):
            ...
            token[nid]    = tokens[j]
        del images, captions
    return img_embs, cap_embs, cap_lens, token, Iou, depend
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    img_embs, cap_embs, cap_lens,_ , Iou, depend = encode_data(model, data_loader)
    ...
    if not fold5:
        ...
        for i in range(5):
            ...
            start = time.time()
            sims = shard_attn_scores(model, img_embs_shard, cap_embs_shard, cap_lens_shard, Iou, depend, opt, shard_size=100)
            end = time.time()
            print("calculate similarity time:", end-start)
...
def evalrank_ensemble(model_path1, model_path2, data_path=None, split='dev', fold5=False):
    ...
    print('Images: %d, Captions: %d' %
          (img_embs.shape[0] / 5, cap_embs.shape[0]))
    img_embs2, cap_embs2, cap_lens2,_ , Iou, depend = encode_data(model2, data_loader)
    print('Images: %d, Captions: %d' %
    ...
    if not fold5:
        ...
        start = time.time()
        sims  = shard_attn_scores(model,  img_embs, cap_embs, cap_lens, Iou, depend, opt, shard_size=100)
        sims2 = shard_attn_scores(model2, img_embs2, cap_embs2, cap_lens2, Iou, depend, opt, shard_size=100)
        end = time.time()
        print("calculate similarity time:", end-start)
        sims = (sims + sims2) / 2
        ...
        for i in range(5):
            ...
            start = time.time()
            sims = shard_attn_scores(model, img_embs_shard, cap_embs_shard, cap_lens_shard, Iou, depend, opt, shard_size=100)
            sims2 = shard_attn_scores(model2,  img_embs_shard2, cap_embs_shard2, cap_lens_shard2, Iou, depend, opt, shard_size=100)
            end = time.time()
            print("calculate similarity time:", end-start)
            sims = (sims + sims2) / 2
            r, rt0 = i2t(img_embs_shard, cap_embs_shard, cap_lens_shard, sims, return_ranks=True)
...
def evalrank_ensemble_three(model_path1, model_path2, model_path3, data_path=None, split='dev', fold5=False):
    """
    Evaluate a trained model on either dev or test. If `fold5=True`, 5 fold
    cross-validation is done (only for MSCOCO). Otherwise, the full glo_data is
    used for evaluation.
    ...
    opt3 = checkpoint3['opt']
    save_epoch3 = checkpoint3['epoch']
    print(opt3)
    ...
    print("=> loaded checkpoint_epoch {}".format(save_epoch))
    print("=> loaded checkpoint_epoch {}".format(save_epoch2))
    print("=> loaded checkpoint_epoch {}".format(save_epoch3))
    
    print('Computing results...')
    img_embs, cap_embs, cap_lens, tokens, Iou, depend = encode_data(model, data_loader)
    print('Images: %d, Captions: %d' %
          (img_embs.shape[0] / 5, cap_embs.shape[0]))
    img_embs2, cap_embs2, cap_lens2, tokens, Iou, depend = encode_data(model2, data_loader)
    print('Images: %d, Captions: %d' %
          (img_embs2.shape[0] / 5, cap_embs2.shape[0]))
    img_embs3, cap_embs3, cap_lens3, tokens, Iou, depend = encode_data(model3, data_loader)
    print('Images: %d, Captions: %d' %
          (img_embs3.shape[0] / 5, cap_embs3.shape[0]))
    
    if not fold5:
        # no cross-validation, full evaluation
        img_embs  = np.array([img_embs[i]  for i in range(0, len(img_embs), 5)])
        img_embs2 = np.array([img_embs2[i] for i in range(0, len(img_embs2), 5)])
        img_embs3 = np.array([img_embs3[i] for i in range(0, len(img_embs3), 5)])
        # record computation time of validation
        start = time.time()
        sims  = shard_attn_scores(model,  img_embs, cap_embs, cap_lens, Iou, depend, opt, shard_size=100)
        sims2 = shard_attn_scores(model2, img_embs2, cap_embs2, cap_lens2,Iou, depend, opt2, shard_size=100)
        sims3 = shard_attn_scores(model3, img_embs3, cap_embs3, cap_lens3,Iou, depend, opt3, shard_size=100)
        end = time.time()
        print("calculate similarity time:", end-start)
        # sims = (sims + sims2 + sims3) / 2
        sims = (sims + sims2 + sims3) / 3
        # bi-directional retrieval
        r, rt = i2t(img_embs, cap_embs, cap_lens, sims, return_ranks=True)
        ri, rti = t2i(img_embs, cap_embs, cap_lens, sims, return_ranks=True)
        ar = (r[0] + r[1] + r[2]) / 3
        ...
        for i in range(5):
            ...
            cap_embs_shard2 = cap_embs2[i * 5000:(i + 1) * 5000]
            cap_lens_shard2 = cap_lens2[i * 5000:(i + 1) * 5000]
            img_embs_shard3 = img_embs3[i * 5000:(i + 1) * 5000:5]
            cap_embs_shard3 = cap_embs3[i * 5000:(i + 1) * 5000]
            cap_lens_shard3 = cap_lens3[i * 5000:(i + 1) * 5000]
            start = time.time()
            sims = shard_attn_scores(model, img_embs_shard, cap_embs_shard, cap_lens_shard, Iou, depend,opt, shard_size=100)
            sims2 = shard_attn_scores(model2,  img_embs_shard2, cap_embs_shard2, cap_lens_shard2, Iou, depend,opt2, shard_size=100)
            sims3 = shard_attn_scores(model3,  img_embs_shard3, cap_embs_shard3, cap_lens_shard3,Iou, depend, opt3, shard_size=100)
            end = time.time()
...
def shard_attn_scores(model, img_embs, cap_embs, cap_lens, Iou, depend, opt, shard_size=100):
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            with torch.no_grad():
                ...
                sim = model.forward_sim(im, ca, l, Iou, depend)
...
# Please use split="testall",
# fold5=False for MSCOCO 5K and split="testall", 
# fold5=True for MSCOCO1K； 
# Do not use split="test", fold5=False for MSCOCO1K
# which only includes 1000 images & 5000 captions 
# and which produces higher but unfair results.
if __name__ == '__main__':
    # MSCOCO
    evalrank_ensemble("./ckpt/coco_BiGRU/checkpoint_SAF_best.pth.tar",
                      "./ckpt/coco_BiGRU/checkpoint_SGR_best.pth.tar",
